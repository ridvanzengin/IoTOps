...
def build_sql_prompt(
    ...
    return (
        ...
        "6. For 'the latest/most recent <value> per <entity>' requests (one row per entity, "
        "its newest reading only), do NOT write `GROUP BY entity_column` with a "
        "non-aggregated value column in SELECT, and do NOT write `HAVING time = MAX(time)` "
        "-- both are invalid SQL (PostgreSQL requires every SELECT/HAVING column to be "
        "either aggregated or in GROUP BY; `time` in a HAVING clause without a matching "
        "GROUP BY on time is neither) and the query will fail to run. Use `DISTINCT ON` "
        "instead, e.g.:\n"
        "   SELECT DISTINCT ON (entity_column) entity_column, time, value_column FROM table\n"
        "   WHERE time >= $__timeFrom AND time <= $__timeTo ORDER BY entity_column, time DESC\n"
        "   PostgreSQL's DISTINCT ON keeps exactly the first row per entity_column under "
        "that ordering -- ordering by `entity_column, time DESC` makes that first row the "
        "newest one.\n\n"
        f"Request: {nl_query}"
    )
...
def build_query_rule_sql_prompt(
    ...
    return (
        ...
        "7. If the request is about each entity's LATEST reading specifically (e.g. 'alert "
        "if the most recent temperature is above 90', as opposed to 'alert if the average "
        "over the last hour is above 90'), do NOT try to express that with `GROUP BY "
        "entity_column HAVING time = MAX(time)` -- that's invalid SQL (a non-aggregated, "
        "non-GROUP-BY column in HAVING) and will fail to run. Use DISTINCT ON in a subquery "
        "to pick each entity's newest row first, then filter on the threshold in the outer "
        "query, e.g.:\n"
        "   SELECT * FROM (\n"
        "     SELECT DISTINCT ON (entity_column) entity_column, time, value_column FROM "
        "table ORDER BY entity_column, time DESC\n"
        "   ) latest WHERE value_column > 90\n\n"
        f"{identifiers_line}"
        f"Request: {nl_query}"
    )
