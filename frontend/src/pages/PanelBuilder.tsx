...
import { addPanel, getDashboard, updatePanel } from "../api/dashboard";
...
import type { Chart, Panel, PanelPosition } from "../types/dashboard";
...
export function PanelBuilder() {
  ...
  async function handleGenerateSql() {
    ...
    setError(null);
    try {
      const { sql: generated } = await generateSql(nlPrompt);
      setSql(generated);
    } catch (err) {
  ...
  async function runQuery(query: string) {
    ...
    setError(null);
    try {
      const queryResult = await queryTelemetrySql({ sql: query, limit: 100 });
      ...
      <div className="panel-builder">
        ...
        <div className="panel-builder__column">
          <div className="wizard-panel">
            ...
            <p className="wizard-panel__hint">Reference only &mdash; for the AI prompt and query above.</p>
            <SchemaBrowser />
          </div>
